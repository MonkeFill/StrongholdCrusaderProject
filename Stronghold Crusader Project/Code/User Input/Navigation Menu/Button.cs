...
public class Button
{
    ...
    public virtual void Draw(SpriteBatch ActiveSpriteBatch) //Drawing the button
    {
        if (BackgroundTexture != null) //If there is a background, draw it
        {
            ...
            Rectangle BackgroundBound = new Rectangle(BackgroundStartX, BackgroundStartY, Width, Height); //Background size and position
            ActiveSpriteBatch.Draw(BackgroundTexture, Position, BackgroundBound, Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f); //actually drawing it
        }
    ...
    public bool Update(MouseState ActiveMouse) //Will return true if active becomes true, handles the logic of the mouse clicking or hovering over it
    {
        if (Visible == true) //Button is visible
        {
            if (Active == false) //Is an active button
            {
    ...
    public void ResetButton() //Resetting the variables of the button
    {
        Active = false;
        Hover = false;
        Visible = true;
